...
export default function CreateTripPage() {
    ...
    const getTripDetail = async() =>{
        ...
        try{
            const response = await axios.get(`https://us-central1-labenu-apis.cloudfunctions.net/labeX/gabriel-mina-cruz/trip/${id}`,{
                headers:{
                    Auth: token
                }
            });
    ...
    const candidatesScreen = candidate.map((candidate)=>{
        return <CandidateCard key={candidate.id} name={candidate.name}  country={candidate.country} applicationText={candidate.applicationText}/>
    })
    return (
        <Conteudo>
            <Viagens>
